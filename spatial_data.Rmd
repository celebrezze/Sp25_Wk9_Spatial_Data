...
# Part 1: Fires and Remote Sensing Data
# Downloading Data
```{r}
...
## ggplot
```{r}
dnirv <- nirv_stack[[19]]-nirv_stack[[18]] # change in NIRv due to fire

ggplot() +
  geom_spatraster(data = dnirv) + # adding raster (easy with tidyterra)
...
ggplot() +
  geom_spatraster(data = dnirv) + # adding raster
...
tm_shape(burn_severity) +
  tm_raster(style = "cat",  # treat as categorical
    ...
    title = "Burn Severity") + # note the plus sign to continue adding elemetns
